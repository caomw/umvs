...
class FPNFeature(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.conv0 = nn.Sequential(
            ...
            # SCConv(3, base_channels, 1, padding=1, dilation=1 , groups=1, pooling_r=1, norm_layer=nn.BatchNorm2D),
            Conv2d(base_channels, base_channels, 3, 1, padding=1),
            # SCConv(base_channels, base_channels, 1, padding=1, dilation=1 , groups=1, pooling_r=1, norm_layer=nn.BatchNorm2D)
        )
        ...
        if self.num_stage == 3:
            ...
            self.out_channels.append(base_channels)
        self.ca = ChannelAttention(final_chs)
    ...
    def forward(self, x):
        ...
        intra_feat = conv2  # (1, 32, 128, 160)
        outputs = {}

        # 通道注意力和空间注意力
        intra_feat = self.ca(intra_feat)

        intra_feat = self.sa(intra_feat) 

        out = self.out1(intra_feat)
        outputs["stage1"] = out
        if self.num_stage == 3:
            intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner1(conv1) # [1, 32, 256, 320]

            out = self.out2(intra_feat) 
            outputs["stage2"] = out


            intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="nearest") + self.inner2(conv0) # [1, 32, 512,640]
...
class DLANetReg(nn.Module):
    def __init__(self, args, in_channels=8, **kwargs):
        super(DLANetReg, self).__init__()
        ...
        # self.block64 = LKA_Attention3d(d_model=64).cuda(0)
        self.block32 = LKA_Attention3d(d_model=32).cuda(0)
        self.block16 = LKA_Attention3d(d_model=16).cuda(0)
    def forward(self, x, **kwargs):
        y = x # [1, 8, 48, 128, 160]
        conv0 = self.conv0(x)
        conv2 = self.conv2(self.conv1(conv0))
        conv4 = self.conv4(self.conv3(conv2))
        x = self.conv6(self.conv5(conv4)) # [1, 8, 48, 128, 160]
       
        x = conv4 + self.conv7(x) # [1, 64, 6, 16, 20]
        x = self.block32(x)
        x = conv2 + self.conv9(x) # [1, 32, ,12, 32, 40]
        x = self.block16(x)
        x = conv0 + self.conv11(x) # [1, 16, 24, 64, 80]
        x = self.prob(x) # [1, 8, 48, 128, 160]
        return x # [1, 1, 48, 128, 160]
...
class CasMVSNet(nn.Module):
    def __init__(self, args):
        ...
        self.regularization = nn.ModuleList([DLANetReg(args), DLANetReg(args), DLANetReg(args)])
...
class UMVSNet(nn.Module):
    def __init__(self, args):
        super(UMVSNet, self).__init__()
